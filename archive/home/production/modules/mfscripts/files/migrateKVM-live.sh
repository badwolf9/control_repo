...
if [ -z $2 ]; then
	echo "Usage : $0 <kvm-name> <dst-server> [--add|--force|--forcenoattach]"
	...
	echo -e "\tIf [force] is given, an unsafe migration will be executed"
	echo -e "\tIf [forcenoattach] is given, an unsafe migration will be executed and no iSCSI attach will be done"
	exit 1	
fi
...
if [ "$ADD" == "--forcenoattach" ]; then
	echo "Migrating $NAME to $DST. No iSCSI attachment will be done! Press return to continue."
	read Y    
else
    if [ "$ADD" == "--add" ]; then
	echo "Step 1.) Adding & Attaching : $DEVLIST on $DST"
	ssh $DST "/root/bin/add+attach.sh $DEVLIST"
    else 
	for i in `echo $DEVLIST`; do
		echo "Step 1.) Attaching : $i on $DST"
		ATTACH=`ssh $DST "/root/bin/attachISCSI.sh $i"`
