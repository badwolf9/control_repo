...
# Last Modified: 27-11-2007 by Robert Waffen (zero0ne@zero0ne.de)
# Last Modified: 12-08-2014 by John Ryan john.ryan@bsse.ethz.ch to include Darwin
#
# Usage: ./check_ram
#
# Description:
...
# and Solaris 8/9 and Mac OSX 10.9, 10.10
#
#
# Extension:
#       * shows cached memory
...
case "$OS" in
        Linux)
            # RedHat 7's 'free' output changed. See: https://access.redhat.com/solutions/406773
            if [ $CODENAME == "Maipo" ];then
                RAM_TOTAL=`free -m | grep  Mem:|awk '{ print $2 }'`
                KBRAM_FREE=`cat /proc/meminfo|grep MemAvailable:|awk '{print $2}'`
                RAM_FREE=$(($KBRAM_FREE/1024))
                RAM_INUSE=$(($RAM_TOTAL-$RAM_FREE))
                RAM_CACHED=`free -m |grep Mem:|awk '{print $6}'`
            else
        ...
        SunOS)
                ...
                ;;
        Darwin)
                RAM_TOTAL=`sysctl -a | grep "hw.memsize: "|awk '{print $(NF)}'`
                RAM_TOTAL=$(($RAM_TOTAL/1000))
                PAGES_FREE=`vm_stat | grep "Pages free:"|awk '{print $(NF)}'|awk -F\. '{print $1}'`
                RAM_FREE=$(($PAGES_FREE*4096/1024))
                RAM_INUSE=$(($RAM_TOTAL-$RAM_FREE))
                ;;
esac
...
PERFDATA="total=${RAM_TOTAL}M used=$PERC_INUSE%;free=$PERC_FREE%;cached=$PERC_CACHED%;$WARN;$CRIT;0;100"
...
if [ $3 ] && [ $3 = "-m" ]; then
        ...
        PERFDATA="total=${RAM_TOTAL}M used=${RAM_INUSE}MB;free=${RAM_FREE}MB;cached=${RAM_CACHED}MB;$RAM_WARN;$RAM_CRIT;0;$RAM_TOTAL"
fi
