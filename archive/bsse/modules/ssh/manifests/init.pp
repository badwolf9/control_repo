...
class ssh (
  ...
  $sshd_config_login_grace_time           = '15',
  ...
  $sshd_gssapistrictacceptorcheck         = 'no',
  ...
  $default_sshd_config_hostkey            = ['/etc/ssh/ssh_host_rsa_key','/etc/ssh/ssh_host_ecdsa_key','/etc/ssh/ssh_host_ed25519_key',],
) {


# Add in the ETH VPZ as the only permitted area in which ssh is allowed.

  $x1 = join(lookup('my_networks.specialServerNets'),",")
  $x2 = join(lookup ('my_networks.nonroutableServerNets'),",")
  $x3 = join(lookup ('my_networks.routableServerNets'),",")
  $x4 = join(lookup ('my_networks.dockNets'),",")
  $x5 = join(lookup ('my_networks.specNets'),",")

  $trustedBSSENets = "${x1},${x2},${x3},${x4},${x5}"
  if $my_debugflag == 'yes' {notify { 'TESTHASH' : message => "$trustedBSSENets" } }
  $trustedIDNets = join(lookup('my_networks.idNets'),",")
  $match_root_login = "       PermitRootLogin without-password"
  ...
  case $::osfamily {
    ...
    'Solaris': {
      $sshd_config_template                    = 'ssh/sshd_config_solaris.erb'
      ...
      case $::kernelrelease {
        '5.11': {
          if $facts['kernelmajversion'] =~ /illumos|oi_151a9/ {
             # openindiana
                         $default_packages     = ['network/ssh']
             } else {
                         $default_packages     = ['network/ssh', 'network/ssh/ssh-utilities', 'service/network/ssh-common']
                    }
#                                                    'network/ssh/ssh-key',
#                                                    'service/network/ssh']
          $default_service_name                  = 'ssh'
  ...
  file  { 'ssh_config' :
    ...
    #content => epp("$ssh_config_template", { 'fqdn' => $facts['fqdn'], 'modtime' => mtime, 'puppetversion' => $facts['puppetversion'], }),
    require => Package[$packages_real],
  }
# use a different template for solaris and openindiana
  file  { 'sshd_config' :
    ensure  => file,
    ...
    #content => epp("$sshd_config_template", { 'fqdn' => $facts['fqdn'], 'modtime' => mtime, 'puppetversion' => $facts['puppetversion'], }),
    require => Package[$packages_real],
  }
