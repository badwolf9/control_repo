...
#    $with_desktop = true
#    notify { "Open VM Tools module for Ubuntu workstation" : }
#    }

  $supported = $openvmtools::params::supported

#    notify { "Open VM Tools module" : }
  ...
  case $::virtual {
    'vmware': {
      if $supported {
        ...
        if $facts['fqdn'] !~ /jpk/ {
        if $with_desktop or 'os_linux_ubuntu_workstation' in $my_classes{
